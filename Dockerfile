...
RUN cd /nexus-repository-composer/; sed -i "s/3.13.0-01/${NEXUS_VERSION}-${NEXUS_BUILD}/g" pom.xml; \
    mvn clean package;
COPY ./nexus-repository-r /nexus-repository-r/
...
ARG R_VERSION=1.0.2
ARG TARGET_DIR_R=/opt/sonatype/nexus/system/org/sonatype/nexus/plugins/nexus-repository-r/${R_VERSION}/
...
COPY --from=build /nexus-repository-r/target/nexus-repository-r-${R_VERSION}.jar ${TARGET_DIR_R}
USER nexus